...
using AutoGen.BasicSample.SemanticKernel;
...
{
    public static IKernelBuilder AddBedrockTextGeneration(
...
string? serviceId = null)
    {
        Func<IServiceProvider, object?, BedrockTextGenerationService> factory = (serviceProvider, _) =>
            new(name, systemMessage, temperature, config);
...
public partial class CodePlugin
{
    [Function(nameof(WriteFile), description: "Writes a file in a solution or project.")]
    [KernelFunction]
        ...
        )
    {
        ...
        return $"File {workingDir}/{relativeFilePath} saved successfully";
    }

    [Function(nameof(ReadAllFiles), description: "Reads all files in a folder or directory recursively.")]
    [KernelFunction]
    [Description("Reads all files in a folder or directory recursively")]
    public async Task<string> ReadAllFiles([Description("The directory we are working in")] string workingDir)
    {
        // Print to the console
        Console.ForegroundColor = ConsoleColor.DarkBlue;
...
public partial class AdminPlugin
{
    [Function(nameof(CreateWorkingDirectory), description: "Creates a working directory.")]
    [KernelFunction]
    [Description("Creates a working directory.")]
    public async Task<string> CreateWorkingDirectory()
    {
        // setup dotnet interactive
        var workDir = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName()).Replace(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
        ...
        return $"Working directory created successfully at: {workDir}";
    }
}


public partial class CreateProjectPlugin
{
    [Function(nameof(CreateDotnetConsoleProject), description: "Creates a new dotnet console project.")]
    [KernelFunction]
    [Description("Creates a new dotnet console project.")]
    public async Task<string> CreateDotnetConsoleProject(
        [Description("The working directory")] string workingDir,
        [Description("The name of the project")] string projectName)
    {
        ...
        return $"Console project {projectName} created.";
    }

    [Function(nameof(CreateDotnetLibraryProject), description: "Creates a new dotnet library project.")]
    [KernelFunction]
    [Description("Creates a new dotnet library project.")]
    public async Task<string> CreateDotnetLibraryProject(
        [Description("The working directory")] string workingDir,
        [Description("The name of the project")] string projectName)
    {
        // Print to the console
        ...
        return $"Library project {projectName} created.";
    }

    [Function(nameof(CreateDotnetSolution), description: "Creates a new dotnet solution.")]
    [KernelFunction]
        ...
        [Description("The name of the solution")] string solutionName)
    {
        ...
        return $"Dotnet Solution {solutionName} created.";
    }

    [Function(nameof(AddDotnetProjectToSolution), description: "Adds a dotnet project to a dotnet solution.")]
    [KernelFunction]
    [Description("Adds a dotnet project to a dotnet solution.")]
    public async Task<string> AddDotnetProjectToSolution(
        [Description("The working directory")] string workingDir,
        [Description("The full path of the project we want to add to the solution")] string projectPath)
    {
        ...
        return $"Project {projectPath} added to Solution.";
    }

    [Function(nameof(AddDotnetProjectReference), description: "Adds a reference from a project to another project.")]
    [KernelFunction]
    [Description("Adds a reference from a project to another project.")]
    public async Task<string> AddDotnetProjectReference(
        [Description("The working directory")] string workingDir,
        ...
        [Description("The name of the project to be referenced")] string projectToBeRefernecedName)
    {
        ...
        return $"Success adding project {projectName} a reference to project {projectToBeRefernecedName}";
    }
}

public partial class CompileCodePlugin
{
    [Function(nameof(CompileDotnetSolution), description: "Compiles a dotnet solution")]
    [KernelFunction]
    [Description("Compiles a dotnet solution")]
    public async Task<string> CompileDotnetSolution([Description("The directory we want to compile")] string workingDir)
    {
        // Print to the console
        Console.ForegroundColor = ConsoleColor.DarkBlue;
        ...
        return "Code compilation finished.";
    }
}

public class ExampleXX_SemanticKernel_CodeGenerator
{
    //private static SemanticKernelAgent CreateAdminKernel(BedrockConfig gptConfig)
    //{
    //    var builder = Kernel.CreateBuilder()
    //        .AddBedrockTextGeneration("admin", "", 0f, gptConfig);
    ...
    //private static SemanticKernelAgent CreateCodeCompilerKernel(BedrockConfig gptConfig)
    //{
    //    var builder = Kernel.CreateBuilder()
    //        .AddBedrockTextGeneration("compiler", "", 0f, gptConfig);
    //    var kernel = builder.Build();
    //    var settings = new OpenAIPromptExecutionSettings
    //    {
    //        ToolCallBehavior = ToolCallBehavior.AutoInvokeKernelFunctions,
    ...
    //private static SemanticKernelAgent CreateCodeBuilderKernel(BedrockConfig gptConfig)
    //{
    //    var builder = Kernel.CreateBuilder()
    //        .AddBedrockTextGeneration("coder", "", 0f, gptConfig);
    //    var kernel = builder.Build();
    //    var settings = new OpenAIPromptExecutionSettings
    //    {
    //        ToolCallBehavior = ToolCallBehavior.AutoInvokeKernelFunctions,
    ...
    //private static SemanticKernelAgent CreateProjectBuilderKernel(BedrockConfig gptConfig)
    //{
    //    var builder = Kernel.CreateBuilder()
    //        .AddBedrockTextGeneration("projectCreator", "", 0f, gptConfig);
    //    var kernel = builder.Build();
    //    var settings = new OpenAIPromptExecutionSettings
    //    {
    //        ToolCallBehavior = ToolCallBehavior.AutoInvokeKernelFunctions,
    ...
    {
        var gpt35Config = LLMConfiguration.GetBedrock();
        var gpt4Config = LLMConfiguration.GetBedrock();
        ...
        // Create projectCreator agent
        var createPlugin = new CreateProjectPlugin();
        var projectCreatorAgent = new AssistantAgent(
            name: "projectCreator",
            systemMessage: """
            You are a Senior C# Engineer that can create console applications, class libraries and solutions in dotnet 8.
            You never generate any code.
            Every time you create a new project you must add it to the solution.
            Don't assign work to other agents.
            """,
            llmConfig: new ConversableAgentConfig
            {
                ...
                {
                    createPlugin.AddDotnetProjectReferenceFunctionContract,
                    createPlugin.AddDotnetProjectToSolutionFunctionContract,
                    createPlugin.CreateDotnetConsoleProjectFunctionContract,
                    createPlugin.CreateDotnetLibraryProjectFunctionContract,
                    createPlugin.CreateDotnetSolutionFunctionContract,
                }
            },
            functionMap: new Dictionary<string, Func<string, Task<string>>>
            {
                { nameof(CreateProjectPlugin.AddDotnetProjectReference), createPlugin.AddDotnetProjectReferenceWrapper },
                { nameof(CreateProjectPlugin.AddDotnetProjectToSolution), createPlugin.AddDotnetProjectToSolutionWrapper },
                { nameof(CreateProjectPlugin.CreateDotnetConsoleProject), createPlugin.CreateDotnetConsoleProjectWrapper },
                { nameof(CreateProjectPlugin.CreateDotnetLibraryProject), createPlugin.CreateDotnetLibraryProjectWrapper },
                { nameof(CreateProjectPlugin.CreateDotnetSolution), createPlugin.CreateDotnetSolutionWrapper },
            })
            .RegisterPrintMessage();
        ...
        var coderAgent = new AssistantAgent(
            name: "coder",
            systemMessage: """
            You are a Senior C# Engineer that can code console applications in dotnet 8.
            You need the working directory in order to perform your tasks.
            Before making any changes you will first read all the files in the working directory.
            Once you know what code you need to create or change you will write it to a file.
            If you need a new project created you must ask the project-creator to create it for you.
            ...
            llmConfig: new ConversableAgentConfig
            {
                ...
                FunctionContracts = new[]
                {
                    new CodePlugin().ReadAllFilesFunctionContract,
                    new CodePlugin().WriteFileFunctionContract,
                }
            },
            functionMap: new Dictionary<string, Func<string, Task<string>>>
            {
                { nameof(CodePlugin.ReadAllFiles), new CodePlugin().ReadAllFilesWrapper },
                { nameof(CodePlugin.WriteFile), new CodePlugin().WriteFileWrapper },
            })
            .RegisterPrintMessage();

        // Create compiler agent
        var compilerAgent = new AssistantAgent(
            name: "compiler",
            systemMessage: """
            You are a Senior C# Engineer that can compile applications in dotnet 8.
            After you compile a code you must report back any errors so that they can be fixed.
            You never create any code.
            """,
            llmConfig: new ConversableAgentConfig
            {
                ...
                FunctionContracts = new[]
                {
                    new CompileCodePlugin().CompileDotnetSolutionFunctionContract
                }
            },
            functionMap: new Dictionary<string, Func<string, Task<string>>>
            {
                { nameof(CompileCodePlugin.CompileDotnetSolution), new CompileCodePlugin().CompileDotnetSolutionWrapper },
            })
            .RegisterPrintMessage();
        ...
        var adminAgent = new AssistantAgent(
            name: "admin",
            systemMessage: """
            You are a lead software engineer who takes coding problem from user and resolves problem by splitting them into small tasks and assign each task to the most appropriate engineer.
            Here's available engineers who you can assign a task to:
                - projectCreator: creates new projects and solutions for dotnet, it also adds references between projects. It doesn't create files.
                - coder: write and save dotnet code to resolve a task. The coder will create the class files.
                - compiler: compiles dotnet code from coder and reports back with any issues.
            
            Create a single working directory using a function.
            All engineers share the same working directory.
            Use the same working directory in all interactions.
            ...
            llmConfig: new ConversableAgentConfig
            {
                ...
                FunctionContracts = new[]
                {
                    new AdminPlugin().CreateWorkingDirectoryFunctionContract,
                }
            },
            functionMap: new Dictionary<string, Func<string, Task<string>>>
            {
                { nameof(AdminPlugin.CreateWorkingDirectory), new AdminPlugin().CreateWorkingDirectoryWrapper },
            })
            .RegisterPrintMessage();

        var groupAdmin = new BedrockAgent(
            name: "groupAdmin",
    ...
    .RegisterPrintMessage();
        ...
        var adminToCoderTransition = Transition.Create(adminAgent, coderAgent, async (from, to, messages) =>
        {
            // the last message should be from admin
            var lastMessage = messages.Last();
            if (lastMessage.From != adminAgent.Name)
            {
                return false;
            }
        ...
        var adminToCompilerTransition = Transition.Create(adminAgent, compilerAgent, async (from, to, messages) =>
        {
            // the last message should be from admin
            var lastMessage = messages.Last();
            if (lastMessage.From != adminAgent.Name)
        ...
        var adminToProjectCreatorTransition = Transition.Create(adminAgent, projectCreatorAgent, async (from, to, messages) =>
        {
            // the last message should be from admin
            var lastMessage = messages.Last();
            if (lastMessage.From != adminAgent.Name)
        ...
        var projectCreatorToAdminTransition = Transition.Create(projectCreatorAgent, adminAgent);
        var coderToAdminTransition = Transition.Create(coderAgent, adminAgent);
        var coderToProjectCreatorTransition = Transition.Create(coderAgent, projectCreatorAgent);
        var projectCreatorToCoderTransition = Transition.Create(projectCreatorAgent, coderAgent);
        var compilerToAdminTransition = Transition.Create(compilerAgent, adminAgent);
        var userToAdminTransition = Transition.Create(userProxy, adminAgent);
        var adminToUserTransition = Transition.Create(adminAgent, userProxy, async (from, to, messages) =>
        {
            // the last message should be from admin
            var lastMessage = messages.Last();
            if (lastMessage.From != adminAgent.Name)
        ...
        var groupChat = new GroupChat(
            admin: groupAdmin,
            members: [adminAgent, coderAgent, compilerAgent, projectCreatorAgent, userProxy],
            workflow: workflow);

        var groupChatManager = new GroupChatManager(groupChat);
        var response = await userProxy.SendAsync(groupChatManager, "Create a working directory");
        ...
        //var response = await adminAgent.GenerateReplyAsync(new List<IMessage> { new MessageEnvelope<ChatMessageContent>(new ChatMessageContent(AuthorRole.User, "Create a temporary folder")) }) as MessageEnvelope<ChatMessageContent>;
